...
public class AgenteSuperInteligente extends AbstractPlayer {
	...
	public AgenteSuperInteligente(StateObservation state, ElapsedCpuTimer timer) {
		...
		for(int i = 0; i<x; i++) {
			for(int j = 0; j<y;j++) {
				switch (mundo.get_pos_type(i,j)) {
					case Pared:
						break;
						//recursos_iniciales.add(new Bloque(new Vector2d(x,y)));;
					case Puerta:
						recursos_iniciales.add(new Puerta(new Vector2d(x,y)));
						break;
					case Player:
						recursos_iniciales.add(new Jugador(new Vector2d(x,y)));
						break;
					case BloqueMovible:
						recursos_iniciales.add(new BloquePiedra(new Vector2d(x,y)));
						break;
					case Llave:
						recursos_iniciales.add(new Llave(new Vector2d(x,y)));
						break;
					case Seta:
						recursos_iniciales.add(new Seta(new Vector2d(x,y)));
						break;
					case Vacio:
						recursos_iniciales.add(new Gujero(new Vector2d(x,y)));
						break;
					case Nothing:
						recursos_iniciales.add(new BloqueLibre(new Vector2d(x,y)));
						break;
				}
			}
		}
		...
		objetivos.add(new Jugador(new Vector2d(1,1)));
		...
		objetivos_finales.add(new Jugador(new Vector2d(1,1)));
